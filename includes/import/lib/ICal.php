...
/**
 * ICS/iCal Parser — adapted for VE Events WordPress Plugin.
 *
 * Originally: https://github.com/u01jmg3/ics-parser
 *
 * @original-author  Jonathan Goode <https://github.com/u01jmg3>
 * @license          https://opensource.org/licenses/mit-license.php MIT License
 * @original-version 3.2.0
 *
 * Modifications:
 * - Namespace changed from ICal to VEV_Import
 * - initUrl() replaced with wp_remote_get() (WordPress HTTP API)
 * - ABSPATH guard added
 */


namespace VEV_Import;
...
class ICal
{
    ...
    {
        if ($this->cal !== array()) {
            trigger_error('ICal::initUrl: Calendar already initialised in constructor', E_USER_NOTICE);
            return $this;
        }

        $args = array(
            'timeout'    => 30,
            'user-agent' => $userAgent ?: 'VE Events Import/1.0 (+https://github.com/marcprobst/ve-events)',
            'sslverify'  => $verifySsl,
        );
        ...
        if ( $statusCode < 200 || $statusCode >= 300 ) {
            throw new \Exception( "VEV Import: ICS URL returned HTTP {$statusCode}." );
        }

        $body = wp_remote_retrieve_body( $response );

        if ( empty( $body ) ) {
            throw new \Exception( 'VEV Import: ICS response body is empty.' );
        }

        $this->initString( $body );
